<?php
add_action( 'wp_ajax_nopriv_add_pit_ajax', 'add_pit' ); // крепим на событие wp_ajax_nopriv_add_object_ajax, где add_object_ajax это параметр action, который мы добавили в перехвате отправки формы, add_object - ф-я которую надо запустить
add_action( 'wp_ajax_add_pit_ajax', 'add_pit' ); // если нужно чтобы вся бадяга работала для админов
...
function add_pit() {
	...
	if ( ! $errors ) { // если с полями все ок, значит можем добавлять пост
		...
		if(get_post_type($post_id) == 'pits' && get_post_status($post_id) == 'pending') {
			$to = get_option('admin_email');
			$subject = 'Було додано нову скаргу - пошкодження дорожного покриття.';
			$message = "Було додано нову скаргу в розділі \"Пошкодження дорожного покриття\", будь ласка перевірте скаргу і опублікуйте або видаліть її.\n
...
// Set the email header which contains the forms name and the email address
	...
	} // если были ошибки, выводим ответ в формате json с success = false и умираем
	else {
		wp_send_json_success( 'Вашу скаргу прийнято до розгляду. Після розгляду вона буде опублікована. 
	Дякуємо за те, що не байдужі до нашого міста!' );
	} // если все ок, выводим ответ в формате json с success = true и умираем

	die(); // умираем :)
}

function on_publish_pits_pending_post( $post ) {
// A function to perform actions when a post is published.

	if ( "pits" === get_post_type() ) { // check the custom post type

		$name   = get_the_title( $post->ID );

		// get email from custom field
		$to = get_field('field_5b000d1d991cb');
...
add_action( "pending_to_publish", "on_publish_pits_pending_post", 10, 1 );
